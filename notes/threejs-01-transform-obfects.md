...
回転の順序で出力結果が変わるため注意が必要！この回転は [[threejs-02-animation]] で学ぶアニメーションと組み合わせることで、オブジェクトを動的に回転させることができます。
...
グループ化しているので position や scale の指定も 1 つ記述すれば 3 つともに反映される。このグループ化の技術は [[threejs-12-haunted-house]] のような実践的なプロジェクトで複数のオブジェクトを効率的に管理するために活用されています
...
オブジェクトの変換の基礎を理解したら、次は [[threejs-02-animation]] でアニメーションを付与する方法や、[[threejs-03-camera]] でカメラの操作方法を学ぶことで、よりインタラクティブなシーンを作成できます。
